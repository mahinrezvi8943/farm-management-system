...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:bits@localhost/farmers'
db=SQLAlchemy(app)
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"UPDATE `register` SET `farmername`='{farmername}',`nidnumber`='{nidnumber}',`age`='{age}',`gender`='{gender}',`phonenumber`='{phonenumber}',`address`='{address}',`farming`='{farmingtype}'")
        post=Register.query.filter_by(rid=rid).first()
...
def register():
    ...
    if request.method=="POST":
        ...
        db.session.commit()
        # query=db.engine.execute(f"INSERT INTO `register` (`farmername`,`nidnumber`,`age`,`gender`,`phonenumber`,`address`,`farming`) VALUES ('{farmername}','{nidnumber}','{age}','{gender}','{phonenumber}','{address}','{farmingtype}')")
